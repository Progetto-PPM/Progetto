/* FUNZIONE PER AGGIUNGERE UNA CLASSE AD UNA SEZIONE DEL TESTO*/
function evidenziaParole(stringa, paroleChiave, enciclopedia, classe1, classe2) {
    // Iteriamo sulle parole chiave
    paroleChiave.forEach(parola => {
      // Creiamo una regex per trovare tutte le occorrenze della parola
      const regex = new RegExp(`\\b${parola}\\b`, 'gi');
      
      // Aggiungiamo la classe alla parola trovata
      stringa = stringa.replace(regex, `<span class="${classe1}">${parola}</span>`);
    });

    enciclopedia.forEach(parola => {
        const regex = new RegExp(`\\b${parola}\\b`, 'gi');

        stringa = stringa.replace(regex, `<span class="${classe2}">${parola}</span>`);
    });
  
    return stringa;
}

const enciclopediasWorlds = ['Sole','Diametro', 'Massa', 'Periodo di Rotazione', "Gravita", "Densita", 'Temperatura Superficiale', 'Distanza dal', 'Periodo di Rivoluzione'];
const classeCSSenciclopedia = 'highlightEnci';
const classeCSSwords = 'highlightWord';
...
const paroleDaCercareSole = ['fonte primaria', 'Nucleo', 'Zona Radiativa', 'Zona Convettiva', 'Cromosfera', 'Corona', 'Fotosfera'];

// Funzione per cambiare il testo
function cambiaTestoSole(indice) {
    // Verifica che l'indice sia valido
    if (indice >= 0 && indice < testiSole.length) {

        testiSole[indice] = testiSole[indice].replace(/\n/g, '<br>');
        // Modifica il contenuto dell'elemento con id "sezioneTesto"
        const testoModificato = evidenziaParole(testiSole[indice], paroleDaCercareSole, enciclopediasWorlds, classeCSSwords, classeCSSenciclopedia);
        document.getElementById('sezioneTesto').innerHTML = testoModificato; // Usa innerHTML per applicare il contenuto HTML modificato
    } else {
        console.error("Indice non valido!"); // Errore se l'indice non è valido
    }
}
/* --------------------------------------------------------------------------- */
...
const paroleDaCercareMercurio = ['Mercurio', 'Sole', 'forte escursione termica', 'ferro', 'superficie', 'scarpate', '60%', 'mantello roccioso', 'esosfera', 'Sistema Solare', 'mancanza', 'vicinanza', '59 giorni', '88', 'sincronia perfetta'];


function cambiaTestoMercurio(indice) {
    if (indice >= 0 && indice < testiMercurio.length) {

        testiMercurio[indice] = testiMercurio[indice].replace(/\n/g, '<br>');
        // Modifica il contenuto dell'elemento con id "sezioneTesto"
        const testoModificato = evidenziaParole(testiMercurio[indice], paroleDaCercareMercurio, enciclopediasWorlds, classeCSSwords, classeCSSenciclopedia);
        document.getElementById('sezioneTesto').innerHTML = testoModificato; // Usa innerHTML per applicare il contenuto HTML modificato
    } else {
        console.error("Indice non valido!"); // Errore se l'indice non è valido
    }
...
const paroleDaCercareVenere = ['Venere', 'Stella del Mattino', 'Sistema Solare', 'ostile', 'densa', 'estreme', 'anidride carbonica', 'azoto', 'pressione atmosferica', '92', 'nucleo', 'campo magnetico', 'mantello roccioso', 'effetto serra', 'opposta', 'collisione', 'luminoso', '900'];


function cambiaTestoVenere(indice) {
    if (indice >= 0 && indice < testiVenere.length) {

        testiVenere[indice] = testiVenere[indice].replace(/\n/g, '<br>');
        // Modifica il contenuto dell'elemento con id "sezioneTesto"
        const testoModificato = evidenziaParole(testiVenere[indice], paroleDaCercareVenere, enciclopediasWorlds, classeCSSwords, classeCSSenciclopedia);
        document.getElementById('sezioneTesto').innerHTML = testoModificato; // Usa innerHTML per applicare il contenuto HTML modificato
    } else {
        console.error("Indice non valido!"); // Errore se l'indice non è valido
    }
...
const paroleDaCercareTerra = ['Terra', 'ecosistema unico', 'biodiversita', 'atmosfera', '71%', '29%', 'placche tettoniche', 'mantello', 'respirazione', 'raggi ultravioletti', 'livelli', 'Nucleo Interno', 'Nucleo Esterno', 'Mantello', 'Crosta', 'campo magnetico'];

function cambiaTestoTerra(indice) {
    if (indice >= 0 && indice < testiTerra.length) {

        testiTerra[indice] = testiTerra[indice].replace(/\n/g, '<br>');
        // Modifica il contenuto dell'elemento con id "sezioneTesto"
        const testoModificato = evidenziaParole(testiTerra[indice], paroleDaCercareTerra, enciclopediasWorlds, classeCSSwords, classeCSSenciclopedia);
        document.getElementById('sezioneTesto').innerHTML = testoModificato; // Usa innerHTML per applicare il contenuto HTML modificato
    } else {
        console.error("Indice non valido!"); // Errore se l'indice non è valido
    }
...
const paroleDaCercareMarte = ['Marte', 'Sistema Solare', 'colonizzazione', 'anidride carbonica', 'azoto', 'argon', '1%', 'Olympus Mons', 'Valles Marineris', '4.000', 'nucleo', 'mantello', 'crosta', 'calotte polari', 'superficie', 'Phobos', 'Deimos', 'asteroidi'];

function cambiaTestoMarte(indice) {
    if (indice >= 0 && indice < testiMarte.length) {

        testiMarte[indice] = testiMarte[indice].replace(/\n/g, '<br>');
        // Modifica il contenuto dell'elemento con id "sezioneTesto"
        const testoModificato = evidenziaParole(testiMarte[indice], paroleDaCercareMarte, enciclopediasWorlds, classeCSSwords, classeCSSenciclopedia);
        document.getElementById('sezioneTesto').innerHTML = testoModificato; // Usa innerHTML per applicare il contenuto HTML modificato
    } else {
        console.error("Indice non valido!"); // Errore se l'indice non è valido
    }
...
const testiSaturno = [
    ...
    "Saturno emette più calore di quanto ne riceva dal Sole, questo è dovuto probabilmente alla compressione del nucleo e al fenomeno di pioggia di elio che, precipitando dal cielo, libera molta energia.\nSaturno esercita una forza gravitazionale costante sui suoi anelli, impedendo che si disperano nello spazio.\nLa bassa densità di Saturno (inferiore all'acqua) significa che, ipoteticamente, galleggerebbe in un oceano d'acqua abbastanza grande da contenerlo."
];

const paroleDaCercareSaturno = ['Saturno', 'superficie', 'Sistema Solare', 'nucleo', 'nove', '70%', 'Atmosfera esterna', 'Strato di idrogeno', 'Nucleo', 'elio', 'energia'];

function cambiaTestoSaturno(indice) {
    if (indice >= 0 && indice < testiSaturno.length) {

        testiSaturno[indice] = testiSaturno[indice].replace(/\n/g, '<br>');
        // Modifica il contenuto dell'elemento con id "sezioneTesto"
        const testoModificato = evidenziaParole(testiSaturno[indice], paroleDaCercareSaturno, enciclopediasWorlds, classeCSSwords, classeCSSenciclopedia);
        document.getElementById('sezioneTesto').innerHTML = testoModificato; // Usa innerHTML per applicare il contenuto HTML modificato
    } else {
        console.error("Indice non valido!"); // Errore se l'indice non è valido
    }
...
const testiUrano = [
    ...
    "L'inclinazione assiale estrema di Urano è probabilmente dovuta ad una collisione con un corpo planetario di dimensioni simili nei primi stadi della sua formazione, che avrebbe ribaltato il pianeta.\nA causa della sua inclinazione, ogni polo di Urano sperimenta 42 anni di luce continui, seguiti da 42 anni di buio.\nUrano emette meno calore rispetto ad altri giganti gassosi, e non si comprende completamente perché sia così freddo rispetto ai suoi simili."
];

const paroleDaCercareUrano = ['Urano', 'Sistema Solare', 'Atmosfera', 'Mantello ghiacciato', 'Nucleo ferroso', 'Distribuzione della temperatura', '42'];

function cambiaTestoUrano(indice) {
    if (indice >= 0 && indice < testiUrano.length) {

        testiUrano[indice] = testiUrano[indice].replace(/\n/g, '<br>');
        // Modifica il contenuto dell'elemento con id "sezioneTesto"
        const testoModificato = evidenziaParole(testiUrano[indice], paroleDaCercareUrano, enciclopediasWorlds, classeCSSwords, classeCSSenciclopedia);
        document.getElementById('sezioneTesto').innerHTML = testoModificato; // Usa innerHTML per applicare il contenuto HTML modificato
    } else {
        console.error("Indice non valido!"); // Errore se l'indice non è valido
    }
...
const paroleDaCercareNettuno = ['Nettuno', 'Sistema Solare', 'superficie', 'nucleo', 'blu', 'processi chimici', '17', 'temperature', '-214°', 'Atmosfera', 'Mantello ghiacciato', 'Nucleo roccioso', 'energia', '40', 'intense'];

function cambiaTestoNettuno(indice) {
    if (indice >= 0 && indice < testiNettuno.length) {

        testiNettuno[indice] = testiNettuno[indice].replace(/\n/g, '<br>');
        // Modifica il contenuto dell'elemento con id "sezioneTesto"
        const testoModificato = evidenziaParole(testiNettuno[indice], paroleDaCercareNettuno, enciclopediasWorlds, classeCSSwords, classeCSSenciclopedia);
        document.getElementById('sezioneTesto').innerHTML = testoModificato; // Usa innerHTML per applicare il contenuto HTML modificato
    } else {
        console.error("Indice non valido!"); // Errore se l'indice non è valido
    }
